                    ...
                    if (user == null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(payload.Picture))
                        {
                            try
                            {
                                ...
                                var cloudResult = await cloudinaryService.UploadUrlToCloudinaryAsync(payload.Picture, "profile_pictures");
                                ...
                                _logger.LogInformation("Uploaded Google profile picture for user {UserId}", user.Id);
                            }
                            catch (Exception ex)
                            {
                                _logger.LogWarning(ex, "Failed to upload Google profile picture for user {UserId}", user.Id);
                            }
                        }
                    }
                    else
