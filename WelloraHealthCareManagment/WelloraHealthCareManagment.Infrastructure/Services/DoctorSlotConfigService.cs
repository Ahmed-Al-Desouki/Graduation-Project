...
namespace WelloraHealthCareManagement.Infrastructure.Services
{
    public class DoctorSlotConfigService : IDoctorSlotConfigService
    {
        ...
        //        // لو update وفيه تغيير حقيقي → regenerate
        //        if (isUpdate && settingsChanged)
        //            await RegenerateSlotsForDayAsync(doctorId, request.DayOfWeek, ct);
        //    }
        //    catch (Exception ex)
        //    {
        //        _logger.LogError(ex,
        //            "Error setting day config for doctor {DoctorId}", doctorId);
        //        throw;
        //    }
        //}
        public async Task SetDayConfigAsync(
            ...
            CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // ── لو في تغيير → regenerate جوا نفس الـ transaction ──
                if (isUpdate && (settingsChanged || wasInactive))
                    await RegenerateSlotsForDayAsync(doctorId, request.DayOfWeek, ct);

                // ── commit لما كل حاجة اشتغلت ─────────────────
                await transaction.CommitAsync(ct);

                _logger.LogInformation(
                    "{Action} config for doctor {DoctorId} day {Day}. SettingsChanged: {Changed}",
                    isUpdate ? "Updated" : "Created",
                    doctorId, request.DayOfWeek, settingsChanged);
            }
            catch (Exception ex)
            {
                // ── لو أي حاجة فشلت → rollback الاتنين مع بعض ──
            ...
            CancellationToken ct)
        {
            ...
            // امسح Available + Blocked (مش Booked)
            ...
            // الـ Booked بس للـ conflict check
            ...
            // جيب الـ exceptions عشان نحسبها في الـ generation
            var exceptions = await _exceptionRepository
            ...
            while (currentDate <= toDate)
            {
                if (currentDate.DayOfWeek == day)
                {
                    ...
                    // لو في day-off لليوم ده → متولدش slots
                    var slotTimes = config.GetEffectiveSlotTimes(exception);

                    var occupied = occupiedByDate.TryGetValue(currentDate, out var occ)
                        ? occ : new List<TimeSlot>();
            ...
            _logger.LogInformation(
                "Regenerated {Count} slots for doctor {DoctorId} day {Day}",
            ...
            CancellationToken ct = default)
        {
            try
            {
                ...
                if (date.Date >= DateTime.UtcNow.Date)
                    await RegenerateSlotsForSingleDateAsync(doctorId, date, ct);
