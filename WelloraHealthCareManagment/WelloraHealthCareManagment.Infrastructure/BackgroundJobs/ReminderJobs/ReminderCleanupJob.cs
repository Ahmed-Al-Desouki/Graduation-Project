...
public class ReminderCleanupJob
{
    ...
    public async Task CleanupAllExpiredRemindersAsync()
    {
        ...
        // Delete all past cache entries globally (housekeeping)
        await cacheRepo.DeleteAllPastOccurrencesAsync(todayUtc);

        // Get all reminders whose last occurrence has passed
        var expiredReminders = await reminderRepo.GetAllExpiredActiveRemindersAsync(now);
        ...
        foreach (var r in expiredReminders)
        {
            try
            {
                // ✅ FIX Issue 5: Direct hard delete — no soft delete, no grace period
                await cacheRepo.DeleteByReminderIdAsync(r.Id);
                await reminderRepo.HardDeleteAsync(r.Id);
                hardDeleted++;

                _logger.LogInformation(
                    "Hard deleted expired reminder {Id} (Type: {Type}, End: {End})",
                    r.Id, r.Type, r.EndDateUtc);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to hard delete reminder {Id}", r.Id);
            }
        }

        _logger.LogInformation(
            "Cleanup finished: {HardCount} reminders hard-deleted", hardDeleted);
    }
}
